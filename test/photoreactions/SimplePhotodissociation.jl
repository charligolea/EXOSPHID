...
@testset verbose=true "SimplePhotodissociation.jl" begin
    ...
    @testset verbose=true "calculate_excess_energy_dissociation_dissociation()" begin
        ...
        Ep = 5.0f0 * eV_to_J
        Eb = 4.0f0 * eV_to_J
        vp = 600.0f0 .* random_unit_tuple()
        st = random_unit_tuple()

        # Case 1: OH but not OH(DPD)
        pn = ("OH(B2Σ)", "O(1S)", "H")
        r1 = PhotoReaction(Eb, vp, st, pn, false)
        E1 = calculate_excess_energy_dissociation(r1, mp, Ep)
        @test isapprox(E1, Ep - get_electronic_energy_predis("OH(B2Σ)") -get_vibrorotational_energy("OH(B2Σ)"); atol=1e-6)

        # Case 2: OH(DPD)
        pn = ("OH(B2Σ)", "O(1S)", "H")
        r2 = PhotoReaction(Eb, vp, st, pn, false)
        E2 = calculate_excess_energy_dissociation(r2, mp, Ep)
        @test isapprox(E2, Ep + 0.5f0*mp*norm(vp)^2 - Eb; atol=1e-6)

        # Case 3: Generic reaction (3 products)
        pn = ("H2O", "OH(A2Σ+)", "H")
        r3 = PhotoReaction(Eb, vp, st, pn, false)
        E3 = calculate_excess_energy_dissociation(r3, mp, Ep)
        E_expected = (Ep + 0.5f0*mp*norm(vp)^2 + get_vibrorotational_energy("H2O")) - Eb - (get_vibrorotational_energy("H") + get_vibrorotational_energy("OH(A2Σ+)"))
                                ...
                                @testset verbose=false "Reaction: $rn | Wavelength range: $wr" begin
                                    ...
                                    if rt == "SPD"
                                        ...
                                        @testset "Positive Excess Energy" begin
                                            for Ep in energs
                                                Ee = calculate_excess_energy_dissociation(r1, m[1], Ep)
                                                @test Ee >= 0
                                            end
                                        end
                                        ...
                                        @testset "Non-zero Velocity Allocation" begin
                                            for Ep in energs
                                                ...
                                                vl, vh = allocate_velocity_dissociation(r1, Ee, m, pp)
                                            tol = 1e-6
                                    ...
                                    elseif rt == "DPD"
                                        ...
                                        @testset "Second Step: Excess Energy" begin
                                            for Ep in energs
                                                Ee = calculate_excess_energy_dissociation(r2, m[1], Ep)
                                                @test Ee >= 0
                                            end
                                        end
                                        ...
                                        @testset "Second Step: Velocity Allocation" begin
                                            for Ep in energs
                                                Ee = calculate_excess_energy_dissociation(r2, m[1], Ep)
                                                pp = calculate_photon_momentum(Ep, st)
                                                vl, vh = allocate_velocity_dissociation(r2, Ee, m, pp)
                                                tol = 1e-6
                                                @test (Ee==0 && norm(vh) < tol && norm(vl) < tol) || (Ee != 0 && (norm(vh) !=0  || norm(vl) !=0))
                                            end
                                        end
