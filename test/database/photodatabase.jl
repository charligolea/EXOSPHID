...
include("../../src/database/solar_database.jl")
using .solar_database


@testset verbose=true "photodatabase.jl" begin
        ...
        for parent_type in exosphid_species
            ...
            for (i,rt) in enumerate(rts)
                if rt in ("SPD", "DPD", "DiPI") 
                    ...
                    elseif rt == "DiPI"
                        for sp in SimplePhotoionisation.get_base_product_names(spns[i][2])
                            if sp ∉ tested_states
                                push!(tested_states, sp)
                                @testset verbose=true "$sp" begin
                                    @testset verbose=true "Available" begin 
    ...
    @testset verbose = true "SPECIES MASSES" begin
        @testset verbose=true "Mass available for given species" begin
            @test length(photodatabase.mass_species) == length(photodatabase.mass_dict)
        end
        @testset verbose=true "Masses are positive and within expected range" begin
            @test all(0 .<= photodatabase.mass_dict .<= 300*photodatabase.m_fund)
        end
