...
struct PhotoReaction
    ...
    function PhotoReaction(E_bond_eV, E_ionisation_eV, parent_velocity, sun_tuple, parameters, wvl_range, energy_vector, product_names)
        E_bond = E_bond_eV * 1.602e-19
        E_ionisation = E_ionisation_eV * 1.602e-19
        product_types = map(s -> replace(s, r"\(.*\)" => ""),product_names)
        new(E_bond_eV, E_ionisation_eV, parent_velocity, sun_tuple, parameters, wvl_range, energy_vector, product_names, product_types, E_bond, E_ionisation)
    end
end
