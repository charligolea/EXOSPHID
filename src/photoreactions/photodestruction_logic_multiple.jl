...
# OBJECTIVE:
- Handles Simple PhotoDissociation (SPD) 

# INPUTS:
- `photon_energy_vector::Vector{Float32}` -> Energies of generated photons, in J
...
- `product_types` -> 2-element vector containing String objects -> Name of the 2 daughter 
    ...
    Only considers "plain" atomic/molecular species, meaning no information about electronic 
        states is given.
"""
function SPD_logic_multiple(photon_energy_vector::Vector{Float32}, 
                            snames::NTuple{3, String}, tsh_energy::Float32, 
                            parent_velocity::Real, sun_tuple)
    product_names = map(s -> replace(s, r"\((?!\+).*?\)" => ""), snames)
    reaction = PhotoReaction(eV2J(tsh_energy), parent_velocity, sun_tuple, snames, true)
...
- `product_types` -> 3-element vector containing String objects -> Name of the 3 daughter 
    ...
    * Currently, this reaction is only supported for the DPD of Water: 
        H2O -> OH + H -> O + H + H
"""
function DPD_logic_multiple(photon_energy_vector::Vector{Float32}, 
...
"""
function SPI_logic_multiple(photon_energy_vector::Vector{Float32}, 
                        snames::NTuple{3, String}, tsh_energy::Float32, 
                        parent_velocity::Real, sun_tuple)
    reaction = PhotoReaction(eV2J(tsh_energy), parent_velocity, sun_tuple, snames, true)
...
- `tsh_energy::NTuple{2, Float32}` -> Ionisation energy for first reaction, and Dissociation 
    Energy for total process, in J
- `parent_velocity::NTuple{3, Float32}` -> 3D velocity Tuple
- `sun_tuple::NTuple{3, Float32})` -> 3D Solar Tuple, or direction of incoming photon
...
"""
function DiPI_logic_multiple(photon_energy_vector::Vector{Float32}, 
                            snames::NTuple{2, NTuple{3, String}}, 
                            tsh_energy::NTuple{2, Float32}, parent_velocity::Real, sun_tuple)
...
"""
    call_photodestruction_logic_multiple(current_reaction, photochemical_info, 
                                        parent_velocity, sun_tuple, photon_energy)
-------------------------------------------------------------------------------------------
...
- `photon_energy_vector::Vector{Float32}` -> Energies of generated photons, in J
"""
function call_photodestruction_logic_multiple(current_reaction::CurrentReaction, 
                                            photochemical_info::Species, 
                                            parent_velocity::Float32, 
    ...
    if pr == "SPD"
        return SPD_logic_multiple(photon_energy_vector, species_names[r_idx], 
                                tsh_energies[r_idx], parent_velocity, sun_tuple)
    
    # H2O Double photodissociation
    elseif pr == "DPD" 
        return DPD_logic_multiple(photon_energy_vector, species_names[r_idx], 
                                tsh_energies[r_idx], parent_velocity, sun_tuple)
    
    # Simple photoionisations
    elseif pr == "SPI"
        return SPI_logic_multiple(photon_energy_vector, species_names[r_idx], 
                                tsh_energies[r_idx], parent_velocity, sun_tuple)

    # Double electron ejection of negative atomic hydrogen
    elseif pr == "DPI"
        return DPI_logic_multiple(photon_energy_vector, species_names[r_idx], 
                                tsh_energies[r_idx], parent_velocity, sun_tuple)

    # Dissociative photoionisations
    elseif pr == "DiPI"
        return DiPI_logic_multiple(photon_energy_vector, species_names[r_idx], 
                                tsh_energies[r_idx], parent_velocity, sun_tuple)
    end
...
function call_photodestruction_logic_multiple(current_reaction::CurrentReaction, 
                                            photochemical_info::Species, 
                                            parent_velocity::Real, 
                                            sun_tuple::Nothing, 
                                            photon_energy_vector::AbstractVector{<:Real})
    return call_photodestruction_logic_multiple(current_reaction, photochemical_info, 
        Float32(parent_velocity), random_unit_tuple(), Float32.(photon_energy_vector))
end

function call_photodestruction_logic_multiple(current_reaction::CurrentReaction,
                                            photochemical_info::Species, 
                                            parent_velocity::Real, 
                                            sun_tuple::AbstractVector{<:Real}, 
                                            photon_energy_vector::AbstractVector{<:Real})
    st = Tuple(Float32.(sun_tuple))
    return call_photodestruction_logic_multiple(current_reaction, photochemical_info, 
        Float32(parent_velocity), st, Float32.(photon_energy_vector))
end
