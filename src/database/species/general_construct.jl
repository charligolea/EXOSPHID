...
Each Species instance collects all relevant parameters for modeling photodestruction and 
    photoionisation channels, as well as their probabilities across wavelength intervals.

# INPUTS

- `wvl_threshold::Float32` -> Threshold wavelength (in Å) above which photodestruction is 
    negligible.
...
- `tsh_energies::Tuple` -> Threshold energies (in eV) for each photoprocess channel. 
    The structure varies depending on the reaction type:
...
- `species_names::Tuple` -> Parent and product species for each channel. The structure 
   varies  depending on the reaction type:
...
- `reaction_names::Tuple` -> Identifiers for each reaction channel (custom labels for 
    later analysis).
...
- `wavelength_range::NTuple{2, Float32}`` -> Photon wavelength intervals (in Å) for 
    which each reaction is active. Each entry is a 2-tuple of Float32 defining the 
...
# TESTING:
- Tests are available to make sure that the photochemical database is correctly built. 
- See `run_tests.jl`
"""
struct Species
