...
# OBJECTIVE: 
- For the cases where parent velocity has been provided as scalar, or solar vector has not 
been provided, generate random unitary vector
"""
...
""" 
    photodestruction(solar_activity, dt, parent_type, parent_velocity, sun_tuple, 
    dist_to_sun)
-------------------------------------------------------------------------------------------
...
- `parent_type::String` -> String with parent molecule type. See `exosphid_species` variable 
in photodatabase.jl for possible species
- `parent_velocity::Tuple{Float32, Float32, Float32}` -> 3D array or Scalar, Parent velocity 
vector in m/s    
- `sun_tuple::Tuple{Float32, Float32, Float32}` -> 3D array or Scalar, Solar Vector 
(== direction of incoming photon)  
- `dist_to_sun::Float32` -> Distance to Sun in A.U. (default set at 1.0 for Earth-moon system 
and NEOs)

# Outputs
- `reaction_occurence::bool` -> true if photoreaction has occured, false if not.
- `reaction_name` -> Identifier for the reaction (e.g. H2O-PD1 is the first PhotoDissociation 
mechanism for H2O ). Empty String "" if `reaction_occurence` is false.
- `product_types` -> ["T1", "T2", "T3"] if `reaction_occurence` is true or [] if 
...
function photodestruction(
    ...
    dt::Float32, 
    parent_type::String, 
    parent_velocity::Tuple{Float32, Float32, Float32}, 
    sun_tuple::Tuple{Float32, Float32, Float32}, 
    dist_to_sun::Float32=1.0f0)
    ...
    k = get_photodestruction_rates(photochemical_info, solar_activity, dist_to_sun)
    reaction_occurence = is_photoreaction_occuring(k, dt)

    if reaction_occurence
        # 3. Generate Photon Energy (J) and Wavelength (Angstrom)
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::Real, sun_tuple::Tuple, dist_to_sun::Real=1.0)
    ...
    return photodestruction(Float32(solar_activity), Float32(dt), parent_type, 
        pv, st, Float32(dist_to_sun))
end
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::Real, sun_tuple::Nothing, dist_to_sun::Real=1.0)
    @assert 0.0 <= parent_velocity "Parent velocity must be positive!"
    pv = Float32(parent_velocity) .* random_unit_tuple()
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::Tuple, sun_tuple::Nothing, dist_to_sun::Real=1.0)
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::Tuple, sun_tuple::Tuple, dist_to_sun::Real=1.0)
    @assert length(parent_velocity) == 3 "Give parent_velocity as a 3D object or a scalar"
    @assert length(sun_tuple) == 3 "Give solar vector as a 3D object! (preferably tuple)"
    pv = Tuple(Float32.(parent_velocity))
    st = Tuple(Float32.(sun_tuple))
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::Real, sun_tuple::AbstractVector{<:Real}, dist_to_sun::Real=1.0)
    @assert 0.0 <= parent_velocity "Parent velocity must be positive!"
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::AbstractVector{<:Real}, sun_tuple::AbstractVector{<:Real}, dist_to_sun::Real=1.0)
...
# ─────────────────────────────────────────────────────────────────────────────────────────
function photodestruction(solar_activity::Real, dt::Real, parent_type::String, 
        parent_velocity::AbstractVector{<:Real}, sun_tuple::Nothing, 
        dist_to_sun::Real=1.0)
    @assert length(parent_velocity) == 3 "parent_velocity must have length 3"
    pv = Tuple(Float32.(parent_velocity))
    st = random_unit_tuple()
    return photodestruction(Float32(solar_activity), Float32(dt), parent_type, 
